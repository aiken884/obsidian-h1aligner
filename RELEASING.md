...
attach `main.js` + `manifest.json` (and `styles.css` if the plugin ever grows
one) as individual binary assets — not inside a zip.
...
- [ ] Check the GitHub Release page shows `main.js` + `manifest.json` as assets.
